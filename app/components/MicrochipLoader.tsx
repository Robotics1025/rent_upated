...
import './MicrochipLoader.css'

interface MicrochipLoaderProps {
    text?: string
    className?: string
}

const MicrochipLoader: React.FC<MicrochipLoaderProps> = ({ text = 'Loading...', className = '' }) => {
    return (
        <div className={`microchip-loader-container ${className}`}>
            <div className="flex flex-col items-center gap-4">
                <svg className="microchip" viewBox="0 0 128 128" width="128px" height="128px" role="img" aria-label="Microchip loader">
                    <symbol id="dot-1">
                ...
                {text && (
                    <p className="text-emerald-600 font-medium animate-pulse">{text}</p>
                )}
            </div>
        </div>
    )
